...
from c_hypermem.schema import SharedNode, TimeBundle, ValidTime, ViewEdge
...
def touch_node_update(node: SharedNode, current_turn: int | None = None) -> SharedNode:
    node.time.lifecycle.updated_at = utc_now_iso()
    node.time.activation.updated_turn = current_turn
    return node


def touch_edge_update(edge: ViewEdge, current_turn: int | None = None) -> ViewEdge:
    edge.time.lifecycle.updated_at = utc_now_iso()
    edge.time.activation.updated_turn = current_turn
    return edge


def touch_node_access(node: SharedNode, current_turn: int | None = None) -> SharedNode:
    node.time.activation.last_access_turn = current_turn
    node.time.activation.access_count += 1
    return node


def touch_edge_access(edge: ViewEdge, current_turn: int | None = None) -> ViewEdge:
