...
```python
{
    ...
    "object": "Company X",
    "qualifiers": {
        "scope_edge_ids": ["edge:coworker_relation"],
        "scope_edge_descriptions": {
            "edge:coworker_relation": "Coworker relation in Company X."
        },
...
这样同一个 Alice 节点的局部三元组可以在不同超边上下文中被解释和过滤。检索组织上下文时可以按 `scope_edge_ids` / `scope_cluster_id` 隔离语义边界，但不会因此改变节点或三元组的 ID 策略。系统追加新的 edge scope 时只更新本轮触达的 triples，不能遍历共享节点后覆盖历史 triples 的上下文。
