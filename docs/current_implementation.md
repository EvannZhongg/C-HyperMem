...
- `content`：按 core edge、sibling edges 的顺序输出 `memoryN：edge description（current_turn_id=...）`，随后直接拼接该 edge 下未重复 node 的 active triples，格式为 `subject -predicate- object`；当 `recall.include_turn_ids_in_context=true` 时，edge 行和 triple 行会额外标注 `source_turn id=`；当 `recall.include_real_time_in_context=true` 时，会额外标注 `real time=`，取值来自来源 turn 的真实插入时间 `turns.inserted_at`。sibling edge 如果包含已在前面输出过的 node，则该 node 的 triples 不重复拼接。
