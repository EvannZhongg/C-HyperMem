...
本文档描述 C-HyperMem 当前代码应遵守的工程架构和后续开发边界。实现细节以 `c_hypermem/` 和 `docs/current_implementation.md` 为准；本文件用于指导后续扩展，不保留旧数据兼容路径。
...
C-HyperMem 是独立 memory 包，不依赖 `agent_memory_eval` 或其他评测框架内部模块。
...
如果需要接入评测框架，只能通过 thin adapter 调用 `c_hypermem.Memory`，并在 adapter 边界完成数据格式转换。C-HyperMem 核心 schema 不应出现 `MemorySession`、`MemoryItem`、LongMemEval、LOCOMO、suite、runner 等评测框架概念。
...
如果接入 `agent_memory_eval`，adapter 应位于评测仓库或 `c_hypermem/adapters/` 可选模块中。职责只包括：
